...
public class PiaggioPolicyPdfExtracter implements Processor {

	private static final Logger logger = LoggerFactory.getLogger(PiaggioPolicyPdfExtracter.class.getName());
	...
	public void process(Exchange exchange) throws Exception {
		logger.info("*********** inside Camel Process of piaggioPolicyPdfExtracter Called ************");
		logger.info("BEGIN : piaggioPolicyPdfExtracter Processor - Started ");
		applicationContext = SpringContext.getAppContext();
		driver = (WebDriver) exchange.getIn().getHeader("chromeDriver_piaggio");
		TransactionInfo transactionInfo = (TransactionInfo) exchange.getProperty(RPAConstants.TRANSACTION_INFO);
		transactionInfo.setProcessPhase(RPAConstants.EXTRACTION);
		PiaggioPolicyPdfExtracter piaggioPolicyPdfExtracter = new PiaggioPolicyPdfExtracter();
		...
		piaggioPolicyPdfExtracter.doProcess(piaggioPolicyPdfExtracter, exchange, transactionInfo, driver, piaggioPolicyService,
				commonService);
		logger.info("*********** inside Camel Process of piaggioPolicyPdfExtracter Processor Ended ************");
	}

	public void doProcess(PiaggioPolicyPdfExtracter piaggioPolicyPdfExtracter, Exchange exchange,
			TransactionInfo transactionInfo, WebDriver driver, PiaggioPolicyService piaggioPolicyService,
			CommonService commonService) throws Exception {
		logger.info("BEGIN : piaggioPolicyPdfExtracter Processor - doProcess Method Called  ");

		getPolicyNumberListTobeExtractedFromLocalDb(driver, transactionInfo, piaggioPolicyService, exchange,
				commonService);
		exchange.setProperty(RPAConstants.TRANSACTION_INFO, transactionInfo);

		logger.info("BEGIN : piaggioPolicyPdfExtracter Processor - doProcess Method Ended  ");
	}

	public boolean getPolicyNumberListTobeExtractedFromLocalDb(WebDriver driver, TransactionInfo transactionInfo,
			PiaggioPolicyService piaggioPolicyService, Exchange exchange, CommonService commonService) throws Exception {
		logger.info(
				"Processor - piaggioPolicyPdfExtracter - BEGIN getPolicyNumberListTobeExtractedFromLocalDb()  called ");
		...
		String hondaFolderPath = "",piaggioTempPDFPath="";
		...
		if (exchange.getProperty(RPAConstants.EXTRACTION_TYPE).equals(RPAConstants.NORMAL)) {
			piaggioPolicyNoList = piaggioPolicyService.findPdfToBeDownloaded(
					commonService.getThresholdFrequencyLevel(RPAConstants.PIAGGIO_DOWNLOAD_THRESHOLD));
		} else {
			String startDate = UtilityFile.getCarPolicyProperty(RPAConstants.PIAGGIO_BACKLOG_STARTDATE);
			String endDate = UtilityFile.getCarPolicyProperty(RPAConstants.PIAGGIO_BACKLOG_ENDDATE);
			piaggioPolicyNoList = piaggioPolicyService.findPdfToBeDownloadedForBackLogPolicies(startDate, endDate,
					commonService.getThresholdFrequencyLevel(RPAConstants.PIAGGIO_DOWNLOAD_THRESHOLD));
		}
		logger.info("piaggioPolicyPdfExtracter - Unextracted Policies count ::" + piaggioPolicyNoList.size());
		...
		if (exchange.getProperty(RPAConstants.EXTRACTION_TYPE).equals(RPAConstants.BACK_LOG)) {
			hondaFolderPath = UtilityFile.getCodeBasePath()
					+ UtilityFile.getCarPolicyProperty(RPAConstants.PIAGGIO_POLICY_FOLDER)
		...
		piaggioTempPDFPath = UtilityFile.getCodeBasePath() + UtilityFile.getCarPolicyProperty("piaggio.policy.temppdf.location");
		File piaggioTempPDFPathFolder = new File(piaggioTempPDFPath);

		if (!piaggioTempPDFPathFolder.exists())
			piaggioTempPDFPathFolder.mkdirs();
		...
		String downloadFilePath = UtilityFile.getCodeBasePath() + UtilityFile.getCarPolicyProperty("piaggio.policy.temppdf.location");
		...
		logger.info(downloadFilePath);
		 ...
		 exchange.getIn().setHeader("chromeDriver_piaggio_fileDownload", newDriver);
		...
		for (PiaggioPolicy piaggioPolicyObj : piaggioPolicyNoList) {
			...
			logger.info("piaggioPolicyPdfExtracter - after switched to main window " + driver.getCurrentUrl());
			...
			logger.info("piaggioPolicyPdfExtracter - after policyNo input set " + driver.getCurrentUrl());

			logger.info("piaggioPolicyPdfExtracter - policyNo ::  " + policyNo);
				 ...
				 if(contentTbl==null){
					 ...
					 if(noRecord==null){
						 ...
						 logger.error("Table Not Loaded even after waited for 60 secs");
						 	piaggioPolicyObj.setIsPolicyDownloaded(RPAConstants.E);
					 		piaggioPolicyObj.setIsProposalDownloaded(RPAConstants.E);
					 }else{
						 logger.info("No record Found for -"+policyNo);
					 		piaggioPolicyObj.setIsPolicyDownloaded(RPAConstants.R);
					 		piaggioPolicyObj.setIsProposalDownloaded(RPAConstants.R);
					 }
				 }else{

			if (driver.getCurrentUrl().contains("Login.aspx")) {
				logger.info("Piaggio Application Logged out automatically");
				setExtractionCount(extractedPolicyCount, extractedProposalCount, transactionInfo, exchange);
				throw new Exception("Piaggio Application Logged out automatically");
			}

			if (piaggioPolicyObj.getIsPolicyDownloaded() == null)
			...
			if (piaggioPolicyObj.getIsPolicyDownloaded().equalsIgnoreCase(RPAConstants.N)) {
				...
				if (saveAsPadf(driver, pdfPath,newDriver)) {
					...
					logger.info("piaggioPolicyPdfExtracter -policy PDF Downloaded for policyNo :: " + policyNo);

					piaggioPolicyObj.setIsPolicyDownloaded(RPAConstants.Y);
					piaggioPolicyObj.setPolicyPdfPath(pdfPath);
					piaggioPolicyObj.setIsPolicyUploaded(RPAConstants.N);
				}
				if(driver!=null && !driver.getCurrentUrl().contains("VSPrintPolicy"))
					driver.close();
			} else {
				logger.info("piaggioPolicyPdfExtracter - Policy pdf already extracted for policy code :: " + policyNo);
			}

			if (piaggioPolicyObj.getIsProposalDownloaded() == null)
			...
			if (piaggioPolicyObj.getIsProposalDownloaded().equalsIgnoreCase(RPAConstants.N)) {
				...
				logger.info("Piaggio - proposalpdflement button clicled");
				...
				if (downloadPdf(driver, phantomjsPath, rasterizeJsPath, policyNo, pdfPath, driver.getCurrentUrl(),
					...
					logger.info(
							"piaggioPolicyPdfExtracter -propsal form PDF Downloaded for proposalNo :: " + proposalNumber);

					piaggioPolicyObj.setIsProposalDownloaded(RPAConstants.Y);
					piaggioPolicyObj.setProposalPdfPath(pdfPath);
				...
				logger.info("piaggioPolicyPdfExtracter - Proposal pdf already extracted for policy code :: " + policyNo);
			}
		}
			 /*}else{
		 		logger.info("No record Found for -"+policyNo);
		...
		logger.info("piaggioPolicyPdfExtracter - Extracted policy count :: " + extractedPolicyCount);

		/* Total proposal pdf extratced */
		transactionInfo.setTotalSuccessUploads((String.valueOf(extractedProposalCount)));
		logger.info("piaggioPolicyPdfExtracter - Extracted Proposal count :: " + extractedProposalCount);

		// At the end, come back to the main window
		driver.switchTo().window(mainWindowHandle);

		logger.info("piaggioPolicyPdfExtracter - Current URL before logout try ::" + driver.getCurrentUrl());
		WebElement logout = UtilityFile.waitForElementPresent(driver, By.xpath("//*[@id='hrefLogOut']"), 40);
		logout.click();
		logger.info("piaggioPolicyPdfExtracter - END getPolicyNumberListTobeExtractedFromLocalDb()");
		return true;
	...
	public boolean deleteFile(String filePath) {
		...
		logger.info("piaggioPolicyPdfExtracter - is File ::" + file.getName() + " Deleted ?? :: " + isFileDeleted);
		file = null;
		return true;
	}
	...
	public boolean saveAsPadf(WebDriver driver, String pdfPath, ChromeDriver newDriver)
		...
		isDeleted = UtilityFile.isFileDeleted(
				UtilityFile.getCodeBasePath() + UtilityFile.getCarPolicyProperty("piaggio.policy.temppdf.location"),
				".pdf");
		if (isDeleted) {
			logger.info(" piaggioPolicyPdfExtracter : saveAsPadf - cleared temp_pdf folder for new Download");
		}else{
			logger.info(" piaggioPolicyPdfExtracter : saveAsPadf - no file available in temp folder to delete");
		}
		...
		do {
			...
			logger.info("piaggioPolicyPdfExtracter : saveAsPadf - isFileDownload_ExistAndRename method called here -- download try number :: "
					+ (i + 1));
			...
			logger.info(" piaggioPolicyPdfExtracter : saveAsPadf - after clicking proposal export pdf  button download try number "+ (i + 1));
			...
			i++;
			if (i == 10) {
				logger.info("piaggioPolicyPdfExtracter : saveAsPadf - file not download after 30 secs of wait ");
				return false;
			}

			}else{
				logger.error("piaggioPolicyPdfExtracter : saveAsPadf - policy pdf element issue  ");
				return false;
			}
		} while (exist != true);
		return true;
