...
public class AbiblApplicationAutomater implements Processor {

	private static final Logger logger = LoggerFactory.getLogger(AbiblApplicationAutomater.class.getName());
	...
	public void process(Exchange exchange)
			throws Exception {
		logger.info("*********** inside Camel Process of abiblApplicationAutomater Called ************");
		logger.info("BEGIN : abiblApplicationAutomater Processor - Started ");
		applicationContext = SpringContext.getAppContext();
		chromeDriverHeadless = applicationContext.getBean(ChromeDriverHeadless.class);
		driver = chromeDriverHeadless.getNewHeadlessDriverWithPdfExternalOpen(UtilityFile.getCodeBasePath() + UtilityFile.getCarPolicyProperty("abibl.policy.temppdf.location"));
		TransactionInfo transactionInfo = (TransactionInfo) exchange.getProperty(RPAConstants.TRANSACTION_INFO);
		transactionInfo.setProcessPhase(RPAConstants.AUTOMATER);
		AbiblApplicationAutomater abiblApplicationAutomater = new AbiblApplicationAutomater();
		exchange.getIn().setHeader("chromeDriver_abibl", driver);
		applicationConfigurationRepository = applicationContext.getBean(ApplicationConfigurationRepository.class);
		abiblApplicationAutomater.doProcess(abiblApplicationAutomater, exchange, transactionInfo, driver,applicationConfigurationRepository);
		logger.info("*********** inside Camel Process of abiblApplicationAutomater Processor Ended ************");
	}

	public void doProcess(AbiblApplicationAutomater abiblApplicationAutomater, Exchange exchange,
			TransactionInfo transactionInfo, WebDriver driver, ApplicationConfigurationRepository applicationConfigurationRepository)
			throws Exception {
		logger.info("BEGIN : abiblApplicationAutomater Processor - doProcess Method Called  ");

		doabiblLogin(driver, transactionInfo,applicationConfigurationRepository);
		exchange.setProperty(RPAConstants.TRANSACTION_INFO, transactionInfo);

		logger.info("BEGIN : abiblApplicationAutomater Processor - doProcess Method Ended  ");
	}

	public boolean doabiblLogin(WebDriver driver, TransactionInfo transactionInfo, ApplicationConfigurationRepository applicationConfigurationRepository) throws Exception {
		logger.info("Processor - abiblApplicationAutomater - BEGIN doabiblLogin()  called ");
		applicationContext = SpringContext.getAppContext();
		...
		if (applicationDetails == null) {
			logger.error("Error in doabiblLogin :: Application details not configured for process name :: "
			...
			executor.executeScript("arguments[0].click();", submit1Element);
				...
				emailService.carPolicyExtractionNotification(transactionInfo,"Unable to locate element - Please check whether the site is reachable","abibl");
			...
			if(isAlertPresent(driver)){
				...
				EmailService emailService = (EmailService) applicationContext.getBean("emailService");
				emailService.carPolicyExtractionNotification(transactionInfo,alert.getText(),"abibl");
				...
				throw new Exception("Invalid Credentials - Unable to Login into Abibl Application");
				...
				JavascriptExecutor executor = (JavascriptExecutor)driver;
				executor.executeScript("arguments[0].click();", expireBtnElem);
				logger.info("abiblApplicationAutomater - Logged Into Abibl Application Successfully::" + driver.getCurrentUrl());
			}else{
				logger.info("abiblApplicationAutomater - Logged Into Abibl Application Successfully::" + driver.getCurrentUrl());
			}
			...
			logger.info("abiblApplicationAutomater - clicked on policyIssuance Element ::" + driver.getCurrentUrl());
			
			logger.info("Processor - abiblApplicationAutomater - END doabiblLogin() ");
			return true;
