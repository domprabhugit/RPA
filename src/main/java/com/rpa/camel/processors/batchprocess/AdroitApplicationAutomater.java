...
public class AdroitApplicationAutomater implements Processor {

	private static final Logger logger = LoggerFactory.getLogger(AdroitApplicationAutomater.class.getName());
	...
	public void process(Exchange exchange)
			throws Exception {
		logger.info("*********** inside Camel Process of AdroitApplicationAutomater Called ************");
		logger.info("BEGIN : AdroitApplicationAutomater Processor - Started ");
		applicationContext = SpringContext.getAppContext();
		chromeDriverHeadless = applicationContext.getBean(ChromeDriverHeadless.class);
		driver = chromeDriverHeadless.getNewHeadlessDriverWithPdfExternalOpen(UtilityFile.getCodeBasePath() + UtilityFile.getVIRProperty("adroit.doc.temppdf.location"));
		TransactionInfo transactionInfo = (TransactionInfo) exchange.getProperty(RPAConstants.TRANSACTION_INFO);
		transactionInfo.setProcessPhase(RPAConstants.AUTOMATER);
		AdroitApplicationAutomater AdroitApplicationAutomater = new AdroitApplicationAutomater();
		exchange.getIn().setHeader("chromeDriver_adroit", driver);
		applicationConfigurationRepository = applicationContext.getBean(ApplicationConfigurationRepository.class);
		AdroitApplicationAutomater.doProcess(AdroitApplicationAutomater, exchange, transactionInfo, driver,applicationConfigurationRepository);
		logger.info("*********** inside Camel Process of AdroitApplicationAutomater Processor Ended ************");
	}

	public void doProcess(AdroitApplicationAutomater AdroitApplicationAutomater, Exchange exchange,
			TransactionInfo transactionInfo, WebDriver driver, ApplicationConfigurationRepository applicationConfigurationRepository)
			throws Exception {
		logger.info("BEGIN : AdroitApplicationAutomater Processor - doProcess Method Called  ");

		doautoInspektLogin(driver, transactionInfo,applicationConfigurationRepository);
		exchange.setProperty(RPAConstants.TRANSACTION_INFO, transactionInfo);

		logger.info("BEGIN : AdroitApplicationAutomater Processor - doProcess Method Ended  ");
	}
	...
	public boolean doautoInspektLogin(WebDriver driver, TransactionInfo transactionInfo, ApplicationConfigurationRepository applicationConfigurationRepository) throws Exception {
		logger.info("Processor - AdroitApplicationAutomater - BEGIN doautoInspektLogin()  called ");
		applicationContext = SpringContext.getAppContext();
		...
		if (applicationDetails == null) {
			...
			WebElement userNameElement = UtilityFile.waitForElementPresent(driver, By.xpath("//*[@id='txtUser']"), 40);
			WebElement passwordElement = UtilityFile.waitForElementPresent(driver, By.xpath("//*[@id='Txtpassword']"), 40);
			WebElement submit1Element = UtilityFile.waitForElementPresent(driver, By.xpath("//*[@id='Button1']"),
					40);
			...
			executor.executeScript("arguments[0].click();", submit1Element);
				...
				emailService.carPolicyExtractionNotification(transactionInfo,"Unable to locate element - Please check whether the site is reachable","adroit");
			...
			if(isAlertPresent(driver)){
				...
				EmailService emailService = (EmailService) applicationContext.getBean("emailService");
				emailService.carPolicyExtractionNotification(transactionInfo,alert.getText(),"adroit");
				...
				throw new Exception("Invalid Credentials - Unable to Login into adroit Application");
				...
				logger.info("AdroitApplicationAutomater - Logged Into autoinspekt Application Successfully::" + driver.getCurrentUrl());
			}*/else{
				logger.info("AdroitApplicationAutomater - Logged Into adroit Application Successfully::" + driver.getCurrentUrl());
			}
			...
			WebElement myTrayElem = UtilityFile.waitForElementPresent(driver,
					By.xpath("//*[@id='ctl00_DataList1_ctl01_HyperLink2']"), 60);
			myTrayElem.click();
			...
			logger.info("AdroitApplicationAutomater - clicked on policyIssuance Element ::" + driver.getCurrentUrl());
			
			logger.info("Processor - AdroitApplicationAutomater - END doautoInspektLogin() ");
			return true;
