...
public class GridWithModelSheetMasterFileReaderProcessor implements Processor

{

	private static final Logger logger = LoggerFactory.getLogger(GridWithModelSheetMasterFileReaderProcessor.class.getName());
	...
	private void AutoWiringBeanPropertiesSetMethod() {
		...
		logger.info("AutoWiringBeanPropertiesSetMethod is Called in GridWithModelSheetMasterFileReaderProcessor Class");
		...
		logger.info("AutoWiringBeanPropertiesSetMethod is Ended in GridWithModelSheetMasterFileReaderProcessor Class");

	}

	@Override
	public void process(Exchange exchange) throws Exception {
		GridWithModelSheetMasterFileReaderProcessor fileReaderProcessor = new GridWithModelSheetMasterFileReaderProcessor();
	...
	private void doProcess(TransactionInfo transactionInfo, Exchange exchange) throws URISyntaxException, IOException {
		...
		logger.info("BEGIN - GridWithModelSheetMasterFileReaderProcessor---doProcess() Method Called Here" );
		transactionInfo.setProcessStatus(RPAConstants.GRID_READER_PHASE);
		readAllFilesInsideAFolderForGridProcess(transactionInfo);
		exchange.setProperty(RPAConstants.TRANSACTION_INFO, transactionInfo);
		logger.info("BEGIN - GridWithModelSheetMasterFileReaderProcessor---doProcess() Method Ended Here" );
	}
...
String filePath = UtilityFile.getCodeBasePath()+ UtilityFile.getGridUploadProperty("GridModel.File.Location.BasePath");
