...
public class AdroitDocUploader implements Processor {

	private static final Logger logger = LoggerFactory.getLogger(AdroitDocUploader.class.getName());

	@Autowired
	ApplicationContext applicationContext;

	@Autowired
	AdroitVirService autoInspektVirService;
	...
	public void process(Exchange exchange) throws Exception {
		...
		AdroitDocUploader autoInspektDocUploader = new AdroitDocUploader();
		autoInspektVirService = applicationContext.getBean(AdroitVirService.class);
	...
	public void doProcess(AdroitDocUploader autoInspektDocUploader, Exchange exchange,
			TransactionInfo transactionInfo, AdroitVirService autoInspektVirService, OmniDocsUploader omniDocUploader,
	...
	public boolean getPolicyNumberListTobeUploadedFromLocalDb(TransactionInfo transactionInfo,
			AdroitVirService autoInspektVirService, Exchange exchange, OmniDocsUploader omniDocUploader,
		...
		List<AdroitVIR> abiblPolicyNoList = null;
		if (exchange.getProperty(RPAConstants.EXTRACTION_TYPE).equals(RPAConstants.NORMAL)) {
			abiblPolicyNoList = autoInspektVirService.findPdfUnUploadedPolicies(
					commonService.getThresholdFrequencyLevel(RPAConstants.AUTOINSPEKT_UPLOAD_THRESHOLD));
		} else {
			abiblPolicyNoList = autoInspektVirService.findPdfUnUploadedBackLogPolicies(
					UtilityFile.getCarPolicyProperty(RPAConstants.AUTOINSPEKT_BACKLOG_STARTDATE),
					UtilityFile.getCarPolicyProperty(RPAConstants.AUTOINSPEKT_BACKLOG_ENDDATE),
					commonService.getThresholdFrequencyLevel(RPAConstants.AUTOINSPEKT_UPLOAD_THRESHOLD));
		}
		...
		int uplodedReportCount = 0,uplodedImageCount=0;
		...
		if (Arrays.stream(environment.getActiveProfiles()).anyMatch(env -> (env.equalsIgnoreCase("prod")))) {
			omniUser = UtilityFile.getOmniDocLiveProperty(RPAConstants.AUTOINSPEKT_OMNI_USERNAME);
			omniPassword = UtilityFile.getOmniDocLiveProperty(RPAConstants.AUTOINSPEKT_OMNI_PASSWORD);
			...
			omniOEMFolderIndex = UtilityFile.getOmniDocLiveProperty(RPAConstants.OMNI_AUTOINSPEKT_FOLDER_INDEX);
			paramValue = RPAConstants.OMNI_AUTOINSPECT_PREFIX_LIVE;
		}else{
			omniUser = UtilityFile.getOmniDocProperty(RPAConstants.AUTOINSPEKT_OMNI_USERNAME);
			omniPassword = UtilityFile.getOmniDocProperty(RPAConstants.AUTOINSPEKT_OMNI_PASSWORD);
			...
			omniOEMFolderIndex = UtilityFile.getOmniDocProperty(RPAConstants.OMNI_AUTOINSPEKT_FOLDER_INDEX);
		}
		...
		try{
			...
			for (AdroitVIR autoInspektVirObj : abiblPolicyNoList) {	
				...
				/*if(autoInspektVirObj.getInwardFolderIndex()==null || autoInspektVirObj.getProposalFolderIndex()==null){
						...
						if(policyFile!=null && policyFile.exists() && !policyFile.isDirectory()) { 
							...
							try{
								...
								carUploadResponse = omniDocUploader.uploadDoc(
										...
										omniDocSize,
										filePath.substring(filePath.lastIndexOf("/") + 1), proposaFolderIndex,
										omnidataDefName,
										omnidataDefId,
										omniPolicyIndex,
										policyNo, inwardCode,
					...
					/* Image upload */
					if (autoInspektVirObj.getIsImageDownloaded().equalsIgnoreCase(RPAConstants.N)) {
						logger.info("autoInspektDocUploader - image doc  not downloaded yet for regNo :: " + regNo);
					} else if (autoInspektVirObj.getIsImageDownloaded().equalsIgnoreCase(RPAConstants.R)) {
						logger.info("autoInspektDocUploader - No Record came while downloaded vir doc for regNo :: " + regNo);
					} else if (autoInspektVirObj.getIsImageDownloaded().equalsIgnoreCase("X")) {
						logger.info("autoInspektDocUploader - Inward / Proposal No is not available to upload for regno :: " + regNo);
					} else if (autoInspektVirObj.getIsImageUploaded().equalsIgnoreCase(RPAConstants.N)
							|| autoInspektVirObj.getIsImageUploaded().equalsIgnoreCase(RPAConstants.D)) {
						carUploadResponse = null;
						logger.info("autoInspektDocUploader - current regNo number to be uploaded " + regNo);
						
						filePath = autoInspektVirObj.getImagePdfPath();
						if(filePath!=null){
						File policyFile = new File(filePath);
						if(policyFile!=null && policyFile.exists() && !policyFile.isDirectory()) { 
							logger.info("autoInspektDocUploader - Image File available in : " + filePath);
							try{
								autoInspektVirObj.setImageUploadedTime(new Date());
								...
								logger.error("autoInspektDocUploader - regNo  - "+regNo+" file not available in " + filePath +"to upload");
								autoInspektVirObj.setIsImageDownloaded(RPAConstants.N);
							}
						}else{
							logger.error("autoInspektDocUploader - file path is empty");
						}
						...
						if (carUploadResponse != null && carUploadResponse.getStatusCode().equals("0")) {
							logger.info("autoInspektDocUploader - regNo status code :: "+carUploadResponse.getStatusCode());
							uplodedImageCount++;
							/* To Delete Image pdf file */
							logger.info("autoInspektDocUploader - is " + regNo + "'s pdf File Deleted ? "
									+ deleteFile(autoInspektVirObj.getImagePdfPath()));
							autoInspektVirObj.setIsImageUploaded(RPAConstants.Y);
						} else {
							if (autoInspektVirObj.getIsImageUploaded().equalsIgnoreCase(RPAConstants.D)) {
								logger.info("autoInspektDocUploader - Unable to upload image pdf for regNo :: "
										+ regNo);
								autoInspektVirObj.setIsImageUploaded(RPAConstants.E);
							} else {
								autoInspektVirObj.setIsImageUploaded(RPAConstants.D);
							}
						}
						if(carUploadResponse!=null){
							autoInspektVirObj.setImageRequest(carUploadResponse.getXmlRequest());
							autoInspektVirObj.setImageResponse(carUploadResponse.getXmlResponse());
						}
					} else {
						logger.info("autoInspektDocUploader - Image pdf already Uploaded for policy code :: " + policyNo);
					}
		...
		logger.info("autoInspektDocUploader - Uploaded Report count :: " + uplodedReportCount);

		/* Total proposal pdf uploaded */
		transactionInfo.setTotalSuccessUploads((String.valueOf(uplodedImageCount)));
		logger.info("autoInspektDocUploader - Uploaded Image count :: " + uplodedImageCount);
