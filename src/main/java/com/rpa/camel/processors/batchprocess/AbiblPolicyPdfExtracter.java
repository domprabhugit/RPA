...
public class AbiblPolicyPdfExtracter implements Processor {

	private static final Logger logger = LoggerFactory.getLogger(AbiblPolicyPdfExtracter.class.getName());
	...
	public void process(Exchange exchange) throws Exception {
		logger.info("*********** inside Camel Process of abiblPolicyPdfExtracter Called ************");
		logger.info("BEGIN : abiblPolicyPdfExtracter Processor - Started ");
		applicationContext = SpringContext.getAppContext();
		driver = (WebDriver) exchange.getIn().getHeader("chromeDriver_abibl");
		TransactionInfo transactionInfo = (TransactionInfo) exchange.getProperty(RPAConstants.TRANSACTION_INFO);
		transactionInfo.setProcessPhase(RPAConstants.EXTRACTION);
		AbiblPolicyPdfExtracter abiblPolicyPdfExtracter = new AbiblPolicyPdfExtracter();
		...
		abiblPolicyPdfExtracter.doProcess(abiblPolicyPdfExtracter, exchange, transactionInfo, driver, abiblPolicyService,
				commonService);
		logger.info("*********** inside Camel Process of abiblPolicyPdfExtracter Processor Ended ************");
	}

	public void doProcess(AbiblPolicyPdfExtracter abiblPolicyPdfExtracter, Exchange exchange,
			TransactionInfo transactionInfo, WebDriver driver, AbiblPolicyService abiblPolicyService,
			CommonService commonService) throws Exception {
		logger.info("BEGIN : abiblPolicyPdfExtracter Processor - doProcess Method Called  ");

		getPolicyNumberListTobeExtractedFromLocalDb(driver, transactionInfo, abiblPolicyService, exchange,
				commonService);
		exchange.setProperty(RPAConstants.TRANSACTION_INFO, transactionInfo);

		logger.info("BEGIN : abiblPolicyPdfExtracter Processor - doProcess Method Ended  ");
	}

	public boolean getPolicyNumberListTobeExtractedFromLocalDb(WebDriver driver, TransactionInfo transactionInfo,
			AbiblPolicyService abiblPolicyService, Exchange exchange, CommonService commonService) throws Exception {
		logger.info(
				"Processor - abiblPolicyPdfExtracter - BEGIN getPolicyNumberListTobeExtractedFromLocalDb()  called ");
		...
		ApplicationConfiguration applicationDetails = commonService.getApplicationDetails(businessProcess.getId(),
				RPAConstants.APPID_ABIBL);
		

		String hondaFolderPath = "",abiblTempPDFPath="";
		...
		if (exchange.getProperty(RPAConstants.EXTRACTION_TYPE).equals(RPAConstants.NORMAL)) {
			abiblPolicyNoList = abiblPolicyService.findPdfToBeDownloaded(
					commonService.getThresholdFrequencyLevel(RPAConstants.ABIBL_DOWNLOAD_THRESHOLD));
		} else {
			String startDate = UtilityFile.getCarPolicyProperty(RPAConstants.ABIBL_BACKLOG_STARTDATE);
			String endDate = UtilityFile.getCarPolicyProperty(RPAConstants.ABIBL_BACKLOG_ENDDATE);
			abiblPolicyNoList = abiblPolicyService.findPdfToBeDownloadedForBackLogPolicies(startDate, endDate,
					commonService.getThresholdFrequencyLevel(RPAConstants.ABIBL_DOWNLOAD_THRESHOLD));
		}
		logger.info("abiblPolicyPdfExtracter - Unextracted Policies count ::" + abiblPolicyNoList.size());
		...
		if (exchange.getProperty(RPAConstants.EXTRACTION_TYPE).equals(RPAConstants.BACK_LOG)) {
			hondaFolderPath = UtilityFile.getCodeBasePath()
					+ UtilityFile.getCarPolicyProperty(RPAConstants.ABIBL_POLICY_FOLDER)
		...
		abiblTempPDFPath = UtilityFile.getCodeBasePath() + UtilityFile.getCarPolicyProperty("abibl.policy.temppdf.location");
		File abiblTempPDFPathFolder = new File(abiblTempPDFPath);

		if (!abiblTempPDFPathFolder.exists())
			abiblTempPDFPathFolder.mkdirs();
		...
		String downloadFilePath = UtilityFile.getCodeBasePath() + UtilityFile.getCarPolicyProperty("abibl.policy.temppdf.location");
		...
		logger.info(downloadFilePath);
		 ...
		 exchange.getIn().setHeader("chromeDriver_abibl_fileDownload", newDriver);
		...
		for (AbiblPolicy abiblPolicyObj : abiblPolicyNoList) {
			...
			logger.info("abiblPolicyPdfExtracter - after switched to main window " + driver.getCurrentUrl());
			...
			logger.info("abiblPolicyPdfExtracter - after policyNo input set " + driver.getCurrentUrl());

			logger.info("abiblPolicyPdfExtracter - policyNo ::  " + policyNo);
				 ...
				 if(contentTbl==null){
					 ...
					 if(noRecord==null){
						 ...
						 logger.error("Table Not Loaded even after waited for 60 secs");
						 	abiblPolicyObj.setIsPolicyDownloaded(RPAConstants.E);
					 		abiblPolicyObj.setIsProposalDownloaded(RPAConstants.E);
					 }else{
						 logger.info("No record Found for -"+policyNo);
					 		abiblPolicyObj.setIsPolicyDownloaded(RPAConstants.R);
					 		abiblPolicyObj.setIsProposalDownloaded(RPAConstants.R);
					 }
				 }else{

			if (driver.getCurrentUrl().contains("Login.aspx")) {
				logger.info("Abibl Application Logged out automatically");
				setExtractionCount(extractedPolicyCount, extractedProposalCount, transactionInfo, exchange);
				throw new Exception("Abibl Application Logged out automatically");
			}

			if (abiblPolicyObj.getIsPolicyDownloaded() == null)
			...
			if (abiblPolicyObj.getIsPolicyDownloaded().equalsIgnoreCase(RPAConstants.N)) {
				...
				if (saveAsPadf(driver, pdfPath,newDriver)) {
					...
					logger.info("abiblPolicyPdfExtracter -policy PDF Downloaded for policyNo :: " + policyNo);

					abiblPolicyObj.setIsPolicyDownloaded(RPAConstants.Y);
					abiblPolicyObj.setPolicyPdfPath(pdfPath);
					abiblPolicyObj.setIsPolicyUploaded(RPAConstants.N);
				}
				if(driver!=null && !driver.getCurrentUrl().contains("VSPrintPolicy"))
					driver.close();
			} else {
				logger.info("abiblPolicyPdfExtracter - Policy pdf already extracted for policy code :: " + policyNo);
			}

			if (abiblPolicyObj.getIsProposalDownloaded() == null)
			...
			if (abiblPolicyObj.getIsProposalDownloaded().equalsIgnoreCase(RPAConstants.N)) {
				...
				logger.info("Abibl - proposalpdflement button clicled");
				...
				if (downloadPdf(driver, phantomjsPath, rasterizeJsPath, policyNo, pdfPath, driver.getCurrentUrl(),
					...
					logger.info(
							"abiblPolicyPdfExtracter -propsal form PDF Downloaded for proposalNo :: " + proposalNumber);

					abiblPolicyObj.setIsProposalDownloaded(RPAConstants.Y);
					abiblPolicyObj.setProposalPdfPath(pdfPath);
				...
				logger.info("abiblPolicyPdfExtracter - Proposal pdf already extracted for policy code :: " + policyNo);
			}
		}
			/*
			}else{
		...
		logger.info("abiblPolicyPdfExtracter - Extracted policy count :: " + extractedPolicyCount);

		/* Total proposal pdf extratced */
		transactionInfo.setTotalSuccessUploads((String.valueOf(extractedProposalCount)));
		logger.info("abiblPolicyPdfExtracter - Extracted Proposal count :: " + extractedProposalCount);

		// At the end, come back to the main window
		driver.switchTo().window(mainWindowHandle);

		logger.info("abiblPolicyPdfExtracter - Current URL before logout try ::" + driver.getCurrentUrl());
		WebElement logout = UtilityFile.waitForElementPresent(driver, By.xpath("//*[@id='hrefLogOut']"), 40);
		logout.click();
		logger.info("abiblPolicyPdfExtracter - END getPolicyNumberListTobeExtractedFromLocalDb()");
		return true;
	...
	public boolean deleteFile(String filePath) {
		...
		logger.info("abiblPolicyPdfExtracter - is File ::" + file.getName() + " Deleted ?? :: " + isFileDeleted);
		file = null;
		return true;
	}
	...
	public boolean saveAsPadf(WebDriver driver, String pdfPath, ChromeDriver newDriver)
		...
		isDeleted = UtilityFile.isFileDeleted(
				UtilityFile.getCodeBasePath() + UtilityFile.getCarPolicyProperty("abibl.policy.temppdf.location"),
				".pdf");
		if (isDeleted) {
			logger.info(" abiblPolicyPdfExtracter : saveAsPadf - cleared temp_pdf folder for new Download");
		}else{
			logger.info(" abiblPolicyPdfExtracter : saveAsPadf - no file available in temp folder to delete");
		}
		...
		do {
			...
			logger.info("abiblPolicyPdfExtracter : saveAsPadf - isFileDownload_ExistAndRename method called here -- download try number :: "
					+ (i + 1));
			...
			if(proposalpdflement!=null){
				...
				logger.info(" abiblPolicyPdfExtracter : saveAsPadf - after clicking proposal export pdf  button download try number "+ (i + 1));
				...
				i++;
				if (i == 10) {
					logger.info("abiblPolicyPdfExtracter : saveAsPadf - file not download after 30 secs of wait ");
					return false;
				}
			}else{
				logger.error("abiblPolicyPdfExtracter : saveAsPadf - policy pdf element issue  ");
				return false;
			}
