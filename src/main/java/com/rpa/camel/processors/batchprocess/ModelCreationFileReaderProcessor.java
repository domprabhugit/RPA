...
public class ModelCreationFileReaderProcessor implements Processor

{

	private static final Logger logger = LoggerFactory.getLogger(ModelCreationFileReaderProcessor.class.getName());
	...
	private void AutoWiringBeanPropertiesSetMethod() {
		...
		logger.info("AutoWiringBeanPropertiesSetMethod is Called in ModelCreationFileReaderProcessor Class");
		...
		logger.info("AutoWiringBeanPropertiesSetMethod is Ended in ModelCreationFileReaderProcessor Class");

	}

	@Override
	public void process(Exchange exchange) throws Exception {
		ModelCreationFileReaderProcessor fileReaderProcessor = new ModelCreationFileReaderProcessor();
	...
	private void doProcess(TransactionInfo transactionInfo, Exchange exchange) throws URISyntaxException, IOException {
		...
		logger.info("BEGIN - ModelCreationFileReaderProcessor---doProcess() Method Called Here" );
		transactionInfo.setProcessStatus(RPAConstants.FILE_PROCESSOR);
		readAllFilesInsideAFolderForGridProcess(transactionInfo);
		exchange.setProperty(RPAConstants.TRANSACTION_INFO, transactionInfo);
		logger.info("BEGIN - ModelCreationFileReaderProcessor---doProcess() Method Ended Here" );
	}
...
FolderConfigurationDetails folderConfigurationDetails = commonService.getFolderSharePath(transactionInfo.getProcessName(), RPAConstants.modelCustomerName, RPAConstants.GridFileType);
...
String filePath = UtilityFile.getCodeBasePath()+ UtilityFile.getGridUploadProperty("modelcreation.File.Location.BasePath");
